...
const EditForm = ({
    ...
    if (options && options.tasksSelector) {
        tasks = semanticUiService.convertDataToSelectOptions(options.tasksSelector);
    }

    let projects;
    if (options && options.projectsSelector) {
        projects = semanticUiService.convertDataToSelectOptions(options.projectsSelector);
    }
